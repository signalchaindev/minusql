...
function rimraf(entry_path) {
  if (!entry_path) {
    throw new Error(chalk__default['default'].red('rimraf requires a path to a directory or file'))
  }

  if (!fs__default['default'].existsSync(entry_path)) {
    console.warn(chalk__default['default'].yellow(`path does not exist: ${entry_path}`));
    return
  }
...
function print_elapsed(start, msg) {
  ...
  console.log(
    `${chalk__default['default'].blue(msg)} ${chalk__default['default'].green('in')} ${chalk__default['default'].blue(elapsed.toFixed(1))}${chalk__default['default'].blue(unit)}`,
  );
}
...
const moduleBinDir = path__default['default'].join(process.cwd(), 'node_modules', 'tempo', '.bin');
const moduleBuildDir = path__default['default'].join(process.cwd(), 'src', 'node_modules');
const modulePackageDir = path__default['default'].join(process.cwd(), 'node_modules', 'tempo', 'src', 'package');
const moduleTempDirPath = path__default['default'].join(process.cwd(), 'node_modules', 'tempo', 'temp');
...
async function buildSchemaAssets(options = {}, actions) {
  ...
  console.log(
    `${chalk__default['default'].blue('[tempo] Compile schema')} ${chalk__default['default'].green('in')} ${chalk__default['default'].blue(
      parseFloat(numbers).toFixed(1) + letters,
    )}`,
  );
  if (stderr) {
    console.error(chalk__default['default'].red(`Error: ${stderr}`));
  }
  ...
  if (actions.ext === '.graphql' || actions.ext === 'any') {
    ...
    const typeDefsPath = path__default['default'].join(process.cwd(), 'src', 'node_modules', '@tempo', 'typeDefs.js');
    rimraf(typeDefsPath);
    ...
    fs__default['default'].writeFileSync(typeDefsPath, `export const typeDefs = \`${printedTypeDefs}\``, 'utf-8');
